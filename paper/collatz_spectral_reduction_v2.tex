...
\hypersetup{
  ...
  pdftitle={A Spectral Reduction of the Collatz Conjecture via Phantom Orbit Shadowing (v2)},
...
\title{
  ...
  \\[4pt]\large\normalfont (Version 2 -- with Lean 4 verification and updated literature)
}
\author{
...
mathematical problems. \emph{Verifying the practical limits of such
collaboration is an explicit secondary goal of this work, on equal
footing with making progress on Collatz.} The methodology, the
contribution split between human and AI, the cross-AI verification
strategy, and the now-completed Lean 4 formalization phase are
documented in Section~\ref{sec:methodology} and in
\texttt{METHODOLOGY.md} of the supplementary archive.
\medskip
...
The Lean formalization \emph{verifies the shadowing core only}: it
does not formalize the episode graph
(Section~\ref{sec:episodes}), the transfer operator
(Section~\ref{sec:operator}), the weighted bound
(Section~\ref{sec:bound}), or the conditional reduction
(Section~\ref{sec:reduction}). Those parts of the paper rest on
deterministic numerical computation and on the conjectures stated
in Section~\ref{sec:reduction}. We discuss this scope explicitly
in Section~\ref{sec:limits} and in
...
\begin{itemize}
  \item Proofs of Conjectures~\ref{conj:T} and~\ref{conj:j}. We
        ...
        operator may itself be hard.
  \item Verification that the empirical phantom set ($k \le 24$) is
        complete in the sense that no SCC outside the listed nodes
        contributes. By \cite[Theorem~7.15]{chang2026}, the
        underlying phantom universe is infinite; verifying that
        no further SCC contributes \emph{at the dynamical level}
        for orbits below an explicit threshold is open.
        Empirical sweeps to $b \le 16$, $t \le 65535$ find none,
        but this is not a proof.
  \item An explicit taxonomic mapping of the empirical phantom
        set used here ($k = 10,\ 11,\ 12$ cycle 1, $12$ cycle 2,
        $20$, see Appendix~\ref{app:phantoms}) onto the universe of
        primitive cyclic compositions enumerated in
        \cite[Theorem~7.15]{chang2026}. This is a finite
        enumeration --- one would tabulate, for each depth $K$ up
        to a chosen cutoff (say $K \le 12$ or $K \le 16$), all
        primitive compositions $(k_1, \ldots, k_\ell)$ with
        expansive drift $\Delta = \ell \log_2 3 - K > 0$, identify
        the ones realised by our four monitored phantoms, and
        verify whether any further primitive composition at the
        same depths produces an SCC of the episode graph not
        already covered. We have not carried out this enumeration
        in the present revision and defer it to subsequent work;
        until it is done, the empirical scope of the SCC analysis
        in Section~\ref{sec:cross} is, strictly, a working
        hypothesis.
  \item A formal version of Theorem~\ref{thm:conditional}.
        Lemma~\ref{lem:shadowing} and Corollary~\ref{cor:no-infinite}
        are formalized in Lean (Section~\ref{sec:formalization}); a
        natural next phase of the program is to formalize the
        episode graph and the transfer operator construction. We
        note explicitly that the \emph{conjectures} themselves
        (Conjectures~\ref{conj:T} and~\ref{conj:j}) involve
        infinite suprema and asymptotic decay, both of which are
        statable but presently unprovable in current Mathlib.
\end{itemize}

\paragraph{Honest claim.}
We have reduced the open structural content of the Collatz
conjecture, at the level of the empirical critical SCC, to a
problem of \emph{spectral perturbation theory on a finite explicit
family of nonnegative matrices}, with the foundational shadowing
lemma mechanically verified, supported by numerical evidence with
a $16\times$ safety margin. We do not claim the conjecture itself.

\subsection{Planned next steps}
\label{sec:planned}

The work above isolates several finite, well-defined targets
whose resolution would either tighten the present reduction or
formalize parts of it that currently rest on numerical evidence.
We list them in increasing order of analytic difficulty.

\paragraph{Priority A --- Phantom-set taxonomy with respect to
\cite[Theorem~7.15]{chang2026}.}
The present empirical phantom set ($k \le 24$,
Appendix~\ref{app:phantoms}) is small relative to the universe of
primitive cyclic compositions enumerated by Chang. A finite but
non-trivial enumeration is in scope: for a chosen depth cutoff
$K_0$ (say $K_0 = 16$ or $20$), enumerate all primitive cyclic
compositions $(k_1, \ldots, k_\ell)$ with expansive drift
$\Delta = \ell \log_2 3 - K > 0$, compute their $2$-adic
representatives $q_w$, simulate sufficiently many integer orbits
shadowing them, and verify either that every resulting
strongly connected component is already covered by the SCC of
Section~\ref{sec:cross}, or otherwise produce explicit new SCCs
to be added. The exercise is bounded and scriptable, but the
M\"obius enumeration of primitive necklaces grows rapidly with
$K_0$ and the per-composition orbit simulation is non-trivial; we
estimate one to two weeks of focused scripting effort before any
honest claim of completeness can be made. This is the first
target we intend to address.

\paragraph{Priority B --- Lean formalization of the episode graph
and the transfer operator.}
The Lean~4 + Mathlib formalization of
Section~\ref{sec:formalization} covers the shadowing core
(Lemma~\ref{lem:shadowing} and Corollary~\ref{cor:no-infinite})
only. A natural extension --- conditional on Priority~A having
fixed the working phantom set --- is to formalize the
episode graph itself: introduce a \texttt{SimpleDigraph} on the
nodes $(k, c, b)$, formalize the SCC computation (e.g.\ via
Tarjan's algorithm or an existing Mathlib graph-theoretic
construction), define the truncated transfer operator
$\full_{T,j}$ and the decomposition
$\full_{T,j} = \core_{T,j} + \tail_{T,j}$, and prove that the
weighted Collatz--Wielandt bound \eqref{eq:bound} is a true
upper bound on $\spec(\full_{T,j})$ for each finite $(T, j)$.
The work is purely combinatorial and does not require new
mathematics, but the Mathlib graph and matrix
infrastructure required is non-trivial; a realistic estimate is
two to four weeks of LLM-assisted Lean sessions, comparable to
the scope of the current Lean phase.

\paragraph{Priority C --- Spectral-gap analysis toward
Conjecture~\ref{conj:T}.}
The strongest analytic target, and the most uncertain, is to
prove a \emph{partial} bound on the family
$\{\full_T\}_{T \ge T_0}$ that does not require full closure of
Conjecture~\ref{conj:T}. The natural tools are a
Lasota--Yorke inequality on a Banach space of $2$-adic
Lipschitz functions, Hennion's theorem to extract a spectral
gap, and Keller--Liverani perturbation
theory~\cite{kellerliverani} to control the dependence on $T$.
Recent infrastructure on quasi-compactness of transfer
operators with countable branches \cite{quasicompact} and
certified spectral approximation \cite{certifiedspectral}
provides a starting point. We do not have the analytic
expertise to carry out this program in isolation and consider it
the most natural target for collaboration with a researcher
working in the area; a single concrete intermediate result,
short of closing Conjecture~\ref{conj:T}, would already be a
meaningful step forward.

We intend to address these in the listed order: A first
(scriptable, contained), then B (Lean infrastructure extension,
contained), then C (open-ended, collaboration-dependent).
...
\paragraph{Lean formalization phase (\texttt{lean/}).}
...
\begin{itemize}
  \item All numerical results in Sections~\ref{sec:numerics} and
        \ref{sec:cross} are deterministic and reproducible from
        the supplementary scripts; anyone with Python 3.10+,
        NumPy, and SciPy can re-run them.
  \item Lemma~\ref{lem:shadowing} and
        Corollary~\ref{cor:no-infinite} are mechanically verified
        in Lean~4 with Mathlib; the project is \texttt{sorry}-free.
        See Section~\ref{sec:formalization} and \texttt{lean/}.
  \item As of this version, the work has not been reviewed by a
        human mathematical expert. The construction of the
        transfer operator, the weighted bound, and the conditional
        reduction theorem rest on AI-assisted formalization that
        has not yet been externally validated. External expert
        review is invited.
  \item The literature reconnaissance is necessarily limited by
        the corpus consulted ($\sim 100$ papers up to and
        including early 2026). Genuinely related work outside
        this corpus is possible; we discuss this risk and a
        mitigation plan in
        Section~\ref{sec:methodology-future-search} below.
\end{itemize}
...
\begin{itemize}
  \item \textbf{For the math.} Read Sections~\ref{sec:shadowing},
        \ref{sec:operator}, and \ref{sec:bound} of this paper. If
        you find an error, an unjustified step, or an implicit
        assumption worth flagging, please contact the author at
...
\begin{description}[leftmargin=4em, style=nextline]
  ...
  \item[\texttt{lean/CollatzShadowing/Auxiliary.lean}]
    ...
    matching property of $q_w$'s own orbit, and periodicity
    identity $B(b L) = b A$.
