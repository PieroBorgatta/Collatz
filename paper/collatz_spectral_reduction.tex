...
This work was developed iteratively in conversation with large
language model assistants (Claude, with comparative checks against
GPT and Gemini at later stages). The conceptual framing
(gravitational debt, $2$-adic corridors, confluence families) and
the strategic decisions about what to investigate are due to the
author. The technical formalization (transfer operator, weighted
Collatz--Wielandt, perturbative decomposition) and the
implementation of the supporting scripts were carried out
collaboratively. The numerical experiments are deterministic and
reproducible from the supplementary scripts; any errors in the
mathematical formulations are the responsibility of the author.
...
\end{center}
and as a frozen Zenodo deposit (DOI to be filled at publication).
