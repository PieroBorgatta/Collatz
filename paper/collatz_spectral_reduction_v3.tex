...
direction of the conjecture). This is qualitatively stronger
(\emph{all} integers vs.\ \emph{almost all}) than the
unconditional ``almost-all'' result of Tao
\cite{tao2019} --- which asserts only logarithmic-density
convergence to almost bounded values. The comparison concerns
the scope of the conclusion, not the quality of the proof: Tao's
result is a deep unconditional theorem, whereas the present
result is a conditional reduction modulo the two stated
finite-matrix estimates. The price of the stronger conclusion is
precisely the open spectral content of
