...
OUTPUT_FILE = "collatz_32_fine_scan_debt_cascade.csv"
SUMMARY_FILE = "collatz_32_summary_fine_scan_debt_cascade.txt"
...
# Scansione fine attorno alla zona migliore trovata:
# lambda=0.35, mu=0.05
LAMBDAS = [round(x / 100, 2) for x in range(30, 41)]  # 0.30 ... 0.40
MUS = [round(x / 100, 2) for x in range(0, 11)]       # 0.00 ... 0.10
...
def export_summary(filename: str, rows: list, elapsed: float):
    ...
    lines.append("FINE SCAN H = log(n) + lambda*Dmax80(n) - mu*Cmax80(n)")
    lines.append("=" * 120)
    lines.append("")
    ...
    lines.append("Candidato precedente da battere:")
    lines.append("lambda=0.35 | mu=0.05 | avg_H=3.081728 | max_H=140 | improved=368286 | worsened=101")
    lines.append("")
...
def main():
    ...
    print("Fine scan debito + cascata")
    print("-" * 120)
    print(f"LIMIT = {LIMIT}")
