...
Derived from TwoSeedDensity.lean, GeneralTargetFrozenSeed.lean and GeneralTargetPositiveDensity.lean.
...
choose the first two congruent seeds above the required height and preserve
the sixth-power mixing decay in the choice of the conductor.
Changes and proofs produced with AI assistance.
-/

import CompactSeedNonreturn
import OptimizedConductor
...
namespace OptimizedDensity

/-- A uniform bound for either candidate, independent of the good residue. -/
def frozenSeedBound (a b N : ℕ) : ℕ :=
  CompactSeedNonreturn.seedBound a
    (ndRootCoreBackwardConductor b (explicitSeedFloor b N / 4) N) b
...
theorem exists_bounded_full_core_mark
    ...
    ∃ (r : ℕ) (hr : Odd r) (hl : 16 ^ b ≤ r),
      r ≤ CompactSeedNonreturn.seedBound a (ndRootCoreBackwardConductor b k n) b ∧
  ...
  obtain ⟨r, hl, hbound, hr, htarget, hres, hnonreturn⟩ :=
    CompactSeedNonreturn.exists_bounded_nonreturning_predecessor_in_residue ha hthree q b residue
...
def conductor (a C : ℕ) : ℕ := OptimizedConductor.conductor (rootBound a C) C
...
theorem predecessor_count_of_mixing
  ...
  let m := conductor a C
  have hm : 1 ≤ m := OptimizedConductor.conductor_one_le T C
  let D : ℝ := OptimizedConductor.scaledConstant (C : ℝ) m
  have hDnonneg : 0 ≤ D := OptimizedConductor.scaledConstant_nonneg (Nat.cast_nonneg C) m
  have hbudget : 88 * (T : ℝ) * D ≤ (2 / 3 : ℝ) * (m : ℝ) ^ 2 :=
    OptimizedConductor.conductor_square_budget T C
  have hsquare : 88 * U.parentSourcePotential * D ≤
      (2 / 3 : ℝ) * (m : ℝ) ^ 2 :=
    (mul_le_mul_of_nonneg_right
      (mul_le_mul_of_nonneg_left hPT (by norm_num : (0 : ℝ) ≤ 88)) hDnonneg).trans hbudget
  have hquadratic : ndExplicitQuadraticMixingAt D m :=
    OptimizedConductor.scaled_quadratic_mixing (Nat.cast_nonneg C) hmix hm
  ...
  have hmass : ∀ n, J ≤ n →
    ...
    exact U.fullGoodCoreTerminalMass_ge_of_square_budget_and_height_of_nonreturningSeed
      ...
      hm hroom.1 hroom.2 hseed hDnonneg hquadratic (by norm_num)
