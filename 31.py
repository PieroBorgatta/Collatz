...
OUTPUT_FILE = "collatz_31_scan_debt_cascade.csv"
SUMMARY_FILE = "collatz_31_summary_debt_cascade.txt"
...
# Lambda principale già individuato.
LAMBDAS = [0.35, 0.40, 0.45, 0.50, 0.55]

# Mu misura quanto premiamo le cascate dissipative future.
MUS = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]
...
def future_metrics(n: int):
    """
    Calcola due grandezze sui prossimi LOOKAHEAD passi:
    ...
    Cmax:
        massima cascata dissipativa futura.
        La definiamo come il massimo surplus:
        C(k) = sum(v2) - k*log2(3)

    Quindi:
        Dmax alto = rischio espansivo futuro
        Cmax alto = presenza di cascata dissipativa futura
    """
    current = n
    cumulative_v2 = 0
...
def H(n: int, lam: float, mu: float):
    m = future_metrics(n)
    return math.log(n) + lam * m["dmax"] - mu * m["cmax"]
...
def analyze_number_for_all_params(start: int):
    """
    Calcola una sola traiettoria e valuta tutte le coppie lambda/mu.
    """
    trajectory = [start]
    n = start
    ...
    for lam in LAMBDAS:
        for mu in MUS:
            start_h_values[(lam, mu)] = (
                start_log
                + lam * start_metrics["dmax"]
                - mu * start_metrics["cmax"]
            )

    h_steps = {(lam, mu): None for lam in LAMBDAS for mu in MUS}
...
def export_summary(filename: str, rows: list, elapsed: float):
    ...
    lines.append("SCAN H = log(n) + lambda*Dmax80(n) - mu*Cmax80(n)")
    lines.append("=" * 120)
    lines.append("")
...
def main():
    ...
    rows.sort(key=lambda r: (r["lambda"], r["mu"]))
