...
## 19. Analisi dei falsi allarmi

Il punto debole osservato a 100 milioni sono i 167 falsi allarmi.

Sono stati quindi eseguiti tre passaggi sperimentali successivi.

### 19.1 Analisi diretta dei falsi allarmi

Lo script 46 ha mostrato che i falsi allarmi sono quasi sempre peggioramenti deboli.
...
| h_delta_at_log_step mediana | circa 0,009769 | sensibilmente maggiore |

Quindi molti falsi allarmi non rappresentano vere esplosioni, ma casi in cui \(H\) rimane appena sopra la quota logaritmica.

### 19.2 Pressione robusta

È stata testata l'ipotesi che \(P_{\max,80}\) fosse troppo sensibile a prefissi corti.

La variante considerata è:
...
Il risultato non giustifica una sostituzione diretta di \(P_{\max,80}\).

La pressione sui prefissi corti non è solo rumore: compare spesso anche nei veri ribelli.

### 19.3 Penalizzazione dei falsi deboli

È stata testata una penalizzazione esplorativa:
...
Il miglior caso ha corretto solo 17 falsi allarmi su 167, ma ha colpito anche 212 veri ribelli su 2.465.

Questa correzione non è selettiva e non viene promossa.

### 19.4 Regole descrittive ex-post

Lo script 48 ha cercato soglie descrittive usando variabili osservate lungo l'orbita.

La migliore regola è:
...
| Classe | Intercettati |
|---|---:|
| FALSO_ALLARME | 107 / 167 |
| INTERMEDIO | 0 / 207 |
| VERO_RIBELLE* | 35 / 2.465 |
| VERO_RIBELLE_ESTREMO | 0 / 399 |

Quindi i falsi allarmi deboli sono parzialmente isolabili, ma non separabili in modo pulito dai veri ribelli non estremi.

Non è stata trovata una regola semplice che intercetti almeno 5 falsi allarmi mantenendo zero estremi e al massimo 10 veri ribelli.

---

## 20. Formula attuale dopo gli esperimenti 46-48

Gli esperimenti 46-48 sono negativi rispetto alla modifica della funzione, ma positivi rispetto alla comprensione del fenomeno.

La conclusione è:

- \(T_{risk}\) non viene promosso;
- \(P^{(m)}_{\max,80}\) non sostituisce \(P_{\max,80}\);
- la penalizzazione \(H_{47}\) non è selettiva;
- le regole ex-post classificano bene una parte dei falsi deboli, ma non sono una nuova funzione di potenziale.

La formula consigliata resta quindi:
