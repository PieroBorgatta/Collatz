...
## 24. Problema aperto: falsi allarmi

A 100 milioni restano 167 falsi allarmi.

Questi casi sono importanti perché indicano dove \(H\) si spaventa senza che la traiettoria sia davvero pericolosa.

La prossima fase della ricerca dovrebbe analizzare i falsi allarmi e verificare se dipendono da:

- prefissi troppo corti con \(P_{\max}=1\);
- \(D_{\max}\) elevato ma non seguito da vera espansione;
- \(C_{\max}\) sottostimato;
- classificazione troppo severa;
- necessità di una pressione regolarizzata.

Una possibile modifica futura è:
...
cioè ignorare prefissi troppo corti nella pressione \(v2=1\).

---

## 25. Stato attuale della teoria
...
Il prossimo obiettivo non è aggiungere nuovi termini, ma capire e ridurre i falsi allarmi, cercando una forma più dimostrabile della pressione 2-adica.
